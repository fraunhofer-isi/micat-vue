...
// NEU: lifecycle / watcher for the results tour
onMounted(() => {
  if (!localStorage.getItem('resultsOnboardingSeen')) {
    showResultsOnboarding.value = true;
  }
});
watch(
...
const cbaData: Ref<Array<CbaData>> = computedAsync(
  async () => {
    ...
    for (const results of session.results) {
      ...
      const filteredDiscountedNewInvestments: number[] = years.map((year, i) => {
        const currentYear = parseInt(year);
        const prevYear = i === 0 ? startingYear - 1 : parseInt(years[i - 1]);
        let sum = 0;
        for (let y = prevYear + 1; y <= currentYear; y++) {
          sum += discountedNewInvestmentsByYear[y] || 0;
        }
        return sum;
      });
...
<template>
  <div class="max-w-screen-xl pt-5 pb-10 mx-auto">
    ...
    <div class="relative my-3 bg-white border border-gray-300 rounded-3xl">
      ...
      <div class="flex">
        ...
        <div v-else-if="activeCategory === 'aggregation'" class="w-full max-w-full p-7">
          <AggregationChart :categories="categories" :cba-data="cbaData"></AggregationChart>
        </div>
    ...
    <OnboardingTour
      v-if="showResultsOnboarding"
      :steps="resultsOnboardingSteps"
      storage-key="resultsOnboardingSeen"
      @close="showResultsOnboarding = false"
    ></OnboardingTour>
  </div>
</template>