...
const getAndStructureGlobalParameters = async (reset: boolean = false) => {
  ...
  const responseGlobalParameters: Response = await fetch(`${import.meta.env.VITE_API_URL}json_parameters?id_mode=${session.future ? 2 : 4}&id_region=${session.region}&orient=records${years}`);
                ...
                >
                  ...
                  <div>
                    <VueNumberFormat
                      ...
                      @change="(e: Event) => entry.value = parseFloat((e.target as HTMLInputElement).value.replace('.', '').replace(',', '.'))"
                    />
                  </div>
                </div>
