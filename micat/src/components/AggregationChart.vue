...
import type { CategoriesInterface, DatasetInterface, ResultInterface, CbaData } from '@/types';
...
const isRelevantMeasurement = (identifier: string) => {
  if (excludedIdentifiers.indexOf(identifier) > -1) return false;
  const isRenewable = session.programs[0]?.type === 'renewable';
  if (!isRenewable && measurementsForRenewables.indexOf(identifier) > -1) return false;
  if (isRenewable && measurementsForEnergyEfficiency.indexOf(identifier) > -1) return false;
  return true;
};
...
const perYearLegendGroups = computed(() => {
  const measurements = props.categories.monetization.measurements.filter((measurement) =>
    isRelevantMeasurement(measurement.identifier)
  );
  return {
...
const perYearChartOptions: any = {
  plugins: {
    ...
    datalabels: {
      display: (context: any) => {
        const datasetsInSameStack = context.chart.data.datasets.filter(
          (d: any) => d.stack === context.dataset.stack && !d.hidden
        );
        if (datasetsInSameStack.length === 0) return false;
        const lastInStack = datasetsInSameStack[datasetsInSameStack.length - 1];
        return (
          context.chart.data.datasets[context.datasetIndex] === lastInStack &&
          !context.dataset.hidden
        );
      },
      formatter: (value: number, context: any) => {
        ...
        return total < 1 && total >= 0
          ? labelFormatterSmall.format(total)
          : labelFormatter.format(total);
      },
      anchor: 'end',
      align: 'end',
      font: { weight: 'normal' }
    ...
    tooltip: {
      callbacks: {
        label: function (context: any) {
          ...
          return `${context.label}: ${value < 0 ? '-' : ''}${labelFormatter.format(
            Math.abs(value)
          )} €/year`;
        }
      }
...
const perYearChartData: any = computed(() => {
  ...
  session.results.forEach((result) => {
    ...
    const measurements = props.categories.monetization.measurements
      .filter((measurement) => isRelevantMeasurement(measurement.identifier))
      .slice()
      .sort((a, b) => {
    ...
    measurements.forEach((measurement) => {
      ...
      if (!result.data[measurement.identifier]) return;
      const aggregationData: ResultInterface = JSON.parse(
        JSON.stringify(result.data[measurement.identifier])
      );
      ...
      aggregationData.rows.forEach((row) => {
        if (measurement.identifier === 'reductionOfEnergyCost') {
          row.splice(0, 2);
        } else if (measurement.identifier === 'reductionOfMortalityMorbidityMonetization') {
          row.splice(0, 2);
        } else {
          row.splice(0, 1);
        }
...
const annuitizedChartData: any = computed(() => {
  ...
  session.results.forEach((result, iP) => {
    ...
    const measurements = props.categories.monetization.measurements.filter(
      (measurement) =>
        isRelevantMeasurement(measurement.identifier) &&
        measurement.identifier !== 'impactOnGrossDomesticProduct'
    );
    ...
    const colors: string[] = [
      'rgb(140, 140, 140)',
      measurementColors.value['impactOnGrossDomesticProduct']
    ];
...
const chartData = computed(() =>
  viewMode.value === 'annuitized' ? annuitizedChartData.value : perYearChartData.value
);
const chartOptions = computed(() =>
  viewMode.value === 'annuitized' ? annuitizedChartOptions : perYearChartOptions
);
</script>
...
<template>
  <div class="p-4 my-5 text-white rounded-lg bg-sky-600 mx-7">
    ...
    <div class="text-sm text-sky-200">
      This tab shows an overview of the monetised indicators. "Per year" splits one-time impacts
      (e.g. GDP effects) and annually recurring impacts into two stacked bars per year. "Annuitized"
      shows the lifetime-annualised equivalent value of investments, GDP and each recurring
      indicator, plus the resulting net annual benefit (as selected and adjusted in the cba module).
      All values are in €.
    </div>
  </div>
  ...
  <div
    v-if="viewMode === 'perYear'"
    class="flex flex-wrap gap-x-6 gap-y-2 mx-7 mb-4 text-xs text-gray-600"
  >
    <div class="flex flex-wrap items-center gap-3">
      ...
      >
        <span
          ...
          :style="{ backgroundColor: measurementColors[m.identifier] }"
        ></span>
        {{ m.title }}
      </button>
    </div>
  ...
  <div
    v-for="(program, i) in session.programs"
    :key="`program-${i}`"
    class="p-4 my-5 rounded-lg bg-gray-50 mx-7"
  >
    <h3 class="mb-2 font-bold text-md">{{ program.name }}</h3>
    <div :id="`aggregation-legend-${i}`"></div>
    <Bar :id="`chart-aggregation-${i}`" :options="chartOptions" :data="chartData[i]" />
  </div>
</template>
