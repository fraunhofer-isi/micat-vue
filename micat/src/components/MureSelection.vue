  ...
  >
    <div class="absolute top-[-14px] left-0 w-full text-center">
      <span class="inline-block px-4 italic font-bold bg-white dark:bg-blue-950 dark:text-white">
        <span>{{
          session.gapFilling
            ? 'Gap Filling Measure Analysis'
            : !session.odyssee
            ? 'Select a policy from MURE'
            : 'Select statistical data from ODYSSEE'
        }}</span>
      </span>
    </div>
    <div class="grid items-center grid-cols-5">
      ...
      <div class="col-span-3">
        ...
        <span class="text-sm font-bold text-gray-800 dark:text-gray-200" v-else>{{
          categories.find((c) => c.id === mureCategory)?.label
        }}</span>
      </div>
      <div class="col-span-2" v-if="!session.gapFilling">
      ...
      <div class="col-span-3" v-if="!session.gapFilling">
        <select
          id="country"
          class="block py-2.5 px-0 w-full text-sm bg-white dark:bg-blue-950 border-0 border-b-2 border-gray-200 appearance-none dark:text-gray-200 dark:border-gray-200 focus:outline-none focus:ring-0 focus:border-gray-200 peer"
          v-model="mureCountry"
        ...
        <span class="text-sm font-bold text-gray-800 dark:text-gray-200" v-else>{{
          countries.find((c) => c.id === mureCountry)?.name
        }}</span>
      </div>
      <div
        class="col-span-2"
        v-if="session.stage === stages.home && !session.odyssee && !session.gapFilling"
      >
        <label for="starting-date" class="text-sm dark:text-white"
          >Starting date <span class="text-xs italic">(optional)</span></label
        >
        <InformationCircleIcon
      ...
      <div class="col-span-3" v-if="session.stage === stages.home && session.odyssee">
        ...
        >
          ...
          <option
            v-for="year in odysseeYears"
            v-bind:key="`odyssee-start-year-${year}`"
            :value="year"
          >
            {{ year }}
          </option>
      ...
      <div class="col-span-3" v-if="session.stage === stages.home && session.odyssee">
        ...
        >
          ...
          <option
            v-for="year in odysseeYears"
            v-bind:key="`odyssee-end-year-${year}`"
            :value="year"
          >
            {{ year }}
          </option>
      ...
      <!-- Gap Filling: Measure dropdown (after sector, before country) -->
      <div class="col-span-2" v-if="session.gapFilling && mureCategory">
        <label for="gf-measurement" class="text-sm dark:text-white">Measure</label>
        <InformationCircleIcon
          @click="openModal('measure')"
          class="inline w-6 h-6 ml-2 cursor-pointer dark:text-white"
        ></InformationCircleIcon>
      </div>
      <div class="col-span-3" v-if="session.gapFilling && mureCategory">
        <select
          id="gf-measurement"
          class="block py-2.5 px-0 w-full text-sm bg-white dark:bg-blue-950 border-0 border-b-2 border-gray-200 appearance-none dark:text-gray-200 dark:border-gray-200 focus:outline-none focus:ring-0 focus:border-gray-200 peer"
          ...
          v-if="session.stage === stages.home"
        >
          <option disabled value="">Select a successful measure</option>
          <option
            v-for="measurement in measurements"
            v-bind:key="`gf-measure-${measurement.id}`"
            :value="measurement.id"
          >
            {{
              truncate(measurement.title, 70) +
              (measurement.country ? ` (${measurement.country.name})` : '')
            }}
          </option>
        </select>
        <span class="text-sm font-bold text-gray-800 dark:text-gray-200" v-else>{{
          measurements.find((m) => m.id === mureMeasurement)?.title
        }}</span>
      </div>
      <!-- Gap Filling: Target country dropdown (after measure) -->
      <div class="col-span-2" v-if="session.gapFilling && mureMeasurement">
        <label for="gf-country" class="text-sm dark:text-white">Target country</label>
        <InformationCircleIcon
          @click="openModal('mure-country')"
          class="inline w-6 h-6 ml-2 cursor-pointer dark:text-white"
        ></InformationCircleIcon>
      </div>
      <div class="col-span-3" v-if="session.gapFilling && mureMeasurement">
        <select
          id="gf-country"
          class="block py-2.5 px-0 w-full text-sm bg-white dark:bg-blue-950 border-0 border-b-2 border-gray-200 appearance-none dark:text-gray-200 dark:border-gray-200 focus:outline-none focus:ring-0 focus:border-gray-200 peer"
          v-model="gapFillingCountry"
          v-if="session.stage === stages.home"
        >
          <option disabled value="">Select a target country</option>
          <option
            v-for="country in countries"
            v-bind:key="`gf-country-${country.id}`"
            :value="country.id"
          >
            {{ country.name }}
          </option>
        ...
        <span class="text-sm font-bold text-gray-800 dark:text-gray-200" v-else>{{
          countries.find((c) => c.id === gapFillingCountry)?.name
        }}</span>
      </div>
      <!-- Regular MURE: Measure dropdown (after country) -->
      <div
        class="col-span-2"
        v-if="!session.gapFilling && mureCategory && mureCountry && !session.odyssee"
      >
      ...
      >
        ...
        >
          ...
          <option
            v-for="measurement in measurements"
            v-bind:key="`country-${measurement.id}`"
            :value="measurement.id"
          >
