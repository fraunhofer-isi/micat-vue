...
export const restructureParameters = (subsectorId: number, name: string, parameters: any) => {
  ...
  for (const [category, dataSet] of Object.entries(parameters)) {
    ...
    for (const data of dataSet as Array<PayloadParameterEntryInterface>) {
      ...
      const entry: ParameterEntry = {
        years: [],
        parameters: {},
        identifier: `${data.id_parameter}-${
          data.id_final_energy_carrier ? data.id_final_energy_carrier : 'na'
        }`
      }
      ...
      if (entry.parameters.id_parameter === 16 || entry.parameters.id_parameter === 67) {
  ...
  if (restructuredResults['fuelSwitch'] && restructuredResults['fuelSwitch'].length > 0) {
    ...
    restructuredResults['fuelSwitch'].sort((a, b) => {
      return a['parameters']['id_final_energy_carrier'] < b['parameters']['id_final_energy_carrier']
        ? -1
        : 1
    })
    // Move efficiency parameters to own category
    restructuredResults['efficiency'] = restructuredResults['fuelSwitch'].filter(
      (entry: ParameterEntry) => {
        return entry.parameters.label.toString().toLowerCase().includes('efficiency')
      }
    )
    restructuredResults['fuelSwitch'] = restructuredResults['fuelSwitch'].filter(
      (entry: ParameterEntry) => {
        return !entry.parameters.label.toString().toLowerCase().includes('efficiency')
      }
    )
  }
  return Object.keys(restructuredResults)
    .sort((a, b) => (a === 'main' ? -1 : 1))
    .reduce((r, k) => ((r[k] = restructuredResults[k]), r), {} as ParameterCategory)
}
export const getGlobalParametersPayload = (
      ...
      for (const [factor, values] of Object.entries(factors)) {
        for (const value of values) {
          ...
          if (category === 'MonetisationFactors') {
            const existingResult: PayloadParameterEntryInterface | undefined = results[
              category
            ].find((result) => {
            ...
            const carrierKey =
              ['ElectricityGeneration', 'HeatGeneration'].indexOf(category) > -1
            ...
            const existingResult: PayloadParameterEntryInterface | undefined = results[
              ...
              if (['ElectricityGeneration', 'HeatGeneration'].indexOf(category) === -1) {
                return (
                  result['id_subsector'] === parseInt(subsector) && result[carrierKey] === value.key
                )
              } else {
            ...
            if (existingResult && value.value !== null) {
              existingResult[factor] =
                ['FuelSplitCoefficient', 'HeatGeneration', 'ElectricityGeneration'].indexOf(
                  category
                ) > -1
                  ? value.value / 100
                  : value.value
              ...
              if (['ElectricityGeneration', 'HeatGeneration'].indexOf(category) === -1)
                data['id_subsector'] = parseInt(subsector)
